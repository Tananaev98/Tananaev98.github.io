...
FOLDER = ROOT / "images/enemies/regions/5_dom_dvor/lvl55"
QA = ROOT / ".codex-image-work/qa_lvl55"
...
def main():
    ...
    sheet.save(QA / "lvl55_sprites.png")
    ...
    medals.save(QA / "lvl55_medallions.png")

    for i, sprite in enumerate(sprites, 1):
        alpha = sprite.getchannel("A")
        print(f"lvl55/{i}: size={sprite.size} mode={sprite.mode} alpha={alpha.getextrema()} bbox={alpha.getbbox()} magenta={magenta_count(sprite)}")
    for i in range(1, 6):
        medal = Image.open(FOLDER / f"{i}{i}.webp")
        print(f"lvl55/{i}{i}: size={medal.size} mode={medal.mode}")
