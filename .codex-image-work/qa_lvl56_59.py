...
QA = ROOT / ".codex-image-work/qa_lvl56_59"
...
def magenta_count(sprite):
    ...
    for y, x in np.argwhere(alpha > 64):
        ...
        hd = min(abs(h * 360 - 320), 360 - abs(h * 360 - 320))
        if hd < 20 and s > 0.55 and v > 0.35:
            count += 1
    return count
...
def main():
    ...
    for level in range(56, 60):
        ...
        for i, sprite in enumerate(sprites, 1):
            ...
            print(f"lvl{level}/{i}: size={sprite.size} mode={sprite.mode} alpha={alpha.getextrema()} bbox={alpha.getbbox()} magenta={magenta_count(sprite)}")
        for i in range(1, 6):
            medal = Image.open(folder / f"{i}{i}.webp")
            print(f"lvl{level}/{i}{i}: size={medal.size} mode={medal.mode}")
